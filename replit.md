...
-   **Manifestação do Destinatário (NT 2020.001)**: Complete system for recipient manifestation with 4 event types, automatic tracking of legal deadlines, and configurable automatic manifestation. Includes logic to prevent invalid manifestations by verifying the recipient.
-   **Hybrid Storage**: Configurable XML storage per company (local or Supabase Storage).
-   **NSU Reconciliation**: Automatic discovery and alignment of the last NSU with SEFAZ, adhering to NT 2014.002 to prevent cStat=656 errors. Implements automatic 65-minute blocking when `ultNSU == maxNSU`.
